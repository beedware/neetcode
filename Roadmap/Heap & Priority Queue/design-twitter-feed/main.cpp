...
int asIntValue(const Value& v){ return (int)v.number; }
vector<int> asIntVectorValue(const Value& v){ vector<int> out; for(const auto& x: v.array) out.push_back((int)x.number); return out; }
vector<string> asStringVectorValue(const Value& v){ vector<string> out; for(const auto& x: v.array) out.push_back(x.str); return out; }
string displayOptionalIntVector(const vector<int>& values){ return displayIntVector(values); }
...
string runExample(const string& example) {
    ...
    while (i < data.size()) {
        ...
        Value arg;
        if (i + 1 < data.size() && data[i + 1].type != Value::STRING) arg = data[i + 1];
        if (false) {
        ...
        i += (i + 1 < data.size() && data[i + 1].type != Value::STRING) ? 2 : 1;
    }
    string out = "[";
...
int main() {
    vector<string> examples = {"[\"Twitter\", \"postTweet\", [1, 10], \"postTweet\", [2, 20], \"getNewsFeed\", [1], \"getNewsFeed\", [2], \"follow\", [1, 2], \"getNewsFeed\", [1], \"getNewsFeed\", [2], \"unfollow\", [1, 2], \"getNewsFeed\", [1]]"};
    vector<string> expected = {"[null, null, null, [10], [20], null, [20, 10], [20], null, [10]]"};
